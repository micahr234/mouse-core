...
- Example training notebooks (`02`, `03`, `05`) replace the degenerate `gamma_step=1.0` / uniform `0.99` boundary discounts with `gamma_step=0.995` and a uniform boundary toll `gamma_boundary = gamma_step ** (MAX_EPISODE_STEPS + 1)` (≈ `0.60` for the 100-step horizon) on all four boundary gammas (episode/task × terminal/truncated). With `gamma_step=1.0`, wall bumps and slow paths tied exactly with optimal play, so greedy action selection was noise and agents walked one direction forever. The steep uniform toll prices every episode boundary — hole, goal, or timeout — at a full episode's worth of discounting, so ending an episode is never a cheap route to future-episode value (the "wormhole effect").
- All example notebooks (`01`–`06`) now share a `MAX_EPISODE_STEPS = 100` constant (previously a hardcoded `max_episode_steps=50` in each env config). The training notebooks derive `gamma_boundary` from it, so the truncation horizon and the boundary discount can't drift apart.
- The experiment runner (`experiments/online_experiment.py`) adopts the same scheme: a new `max_episode_steps` config field (default `100`, exposed as `--max-episode-steps`) replaces the hardcoded `50`, `gamma_step` defaults to `0.995`, and the `gamma_episode` field is gone — all four boundary discounts are derived as `gamma_step ** (max_episode_steps + 1)`. `experiments/summarize.py` prints `max_episode_steps` instead of the removed field.
- `examples/04_inference.ipynb` evaluates over an **episode budget** instead of a step budget: each env runs until it completes `EVAL_EPISODES` episodes (replacing `TOTAL_STEPS` env steps) and reports total points over that budget. This matches the metric the training discounts encode — a fixed number of in-context episodes where a hole and a timeout each cost exactly one episode — instead of a fixed step count where dying early frees up budget. Replay capture is capped at `VIDEO_FRAMES` frames per env.
