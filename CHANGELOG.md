...
- Example notebooks restructure the problem around **finite tasks**: every env runs `episodes_per_task=5` with `task_reset_options={"regenerate_map": True}`, so each task is a fresh procedural map (with a fresh id relabeling) and a budget of 5 episodes to figure it out in context. `MAX_EPISODE_STEPS` drops to `30` (was hardcoded `50`, briefly `100`), making the truncation deadline the only within-episode progress pressure.
...
- `examples/04_inference.ipynb` evaluates a **task budget**: each env runs `TASKS_PER_ENV` (4) held-out tasks and reports points per task (0–5) plus the mean from `env.metrics.task_cum_rewards`. The eval loop keeps per-env current-task row lists and rebuilds the shared batched KV cache with one ragged prefill whenever any env crosses a task boundary (done code 3/4), so context never leaks across maps. Replay capture is capped at `MAX_VIDEO_FRAMES` frames per env.
