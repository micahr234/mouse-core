...
- ``value_gap_gate(beta=, eps=)``: DQN continuation
  ``c = exp(-beta * (V - Q(s, a_taken)) / (max_a Q - min_a Q + eps))``
  with ``V = max_a Q`` on detached online Q (after ``value``). A zero
  gap continues (``c = 1``); a larger relative gap bootstraps the
  delayed state value. ``examples/14_train_offline_value_gap_dqn.ipynb``
  is the same offline loop as ``02`` with this gate.
