...
| [12 — Offline Retrace(λ) DQN](examples/12_train_offline_retrace.ipynb) | Same offline loop as `02`, with `RetraceObjective` ([Munos et al., 2016](https://arxiv.org/abs/1606.02647)): λ-return whose trace is cut by `min(1, π/μ)`, `π = softmax(Q / temperature)` on the delayed Q (`0` = greedy); `μ` is not stored — a second `DiscreteActionHead` (`behavior`) learns it by behavior cloning inside the same objective |
