...
The delayed DQN model is `model.delayed_copy(heads=("action_value",))`: a frozen copy of the online model in which every trainable parameter is copied and every frozen parameter (the bf16 LoRA base) is shared by reference, carrying only the heads the target reads (a behavior or policy head whose delayed values nothing uses is left out). It runs on the same `TokenBatch` as the online model, and `Polyak(model, delayed_model).update(tau_heads=..., tau_encoder=..., tau_backbone=...)` interpolates each section in fp32 with its own τ after every optimizer step (`0` frozen, `1` copy of the online weights); the heads section covers only the heads the delayed model has. `ExponentialDecay` and `Piecewise` (`from mouse_core import ExponentialDecay, Piecewise`) schedule such scalars over optimizer steps.
...
| [12 — Offline Retrace(λ) DQN](examples/12_train_offline_retrace.ipynb) | Same offline loop as `02`, with `RetraceObjective` ([Munos et al., 2016](https://arxiv.org/abs/1606.02647)): λ-return whose trace is cut by `min(1, π/μ)`, `π = softmax(Q / temperature)` on the delayed Q (`0` = greedy); `μ` is not stored — a second `DiscreteActionHead` (`behavior`) outputs logits fit by NLL to the taken actions inside the same objective, and its softmax is `μ` |
...
- **fp32 LoRA on a frozen bf16 base** — `Qwen3Backbone(train_kernel="flex", decode_kernel="flex", dtype=preferred_dtype(device), use_norm=True, pretrained="Qwen/Qwen3-0.6B", lora=LoRAConfig(rank=16, alpha=32))`: on CUDA the frozen base is **bfloat16** and runs FlexAttention, while LoRA adapters on every backbone `nn.Linear`, plus the encoder, reasoner / recurrence, and heads, are float32.
