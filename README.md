...
- **fp32 LoRA on a frozen bf16 base** — `Qwen3Backbone(train_kernel="varlen", decode_kernel="flex", dtype=preferred_dtype(device), pretrained="Qwen/Qwen3-0.6B", lora=LoRAConfig(rank=16, alpha=32))`: on CUDA the frozen base is **bfloat16** and runs through the FlashAttention varlen kernel, while the LoRA adapters, encoder, reasoner / recurrence, and heads are float32.
