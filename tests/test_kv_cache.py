...
def test_batched_cached_rollout_matches_per_row_rollout() -> None:
    ...
    with torch.no_grad():
        ...
        reference = torch.stack(per_row)  # [B, S, A]

        cache = None
        batched_per_step = []
        for s in range(6):
            preds, _, cache = model(
                [[row[s]] for row in rows], cache=cache, use_cache=True
            )
            batched_per_step.append(preds["action_value"][:, -1])
        batched = torch.stack(batched_per_step, dim=1)  # [B, S, A]
...
def test_ragged_batched_chunks_match_unbatched(backbone_cls, tokens) -> None:
    ...
    # Steps contributed by each of the 3 rows on each of the 3 calls.
    chunk_lengths = [
        [2, 4, 3],  # ragged prefill
        [3, 0, 2],  # middle row contributes nothing this call
        [1, 3, 1],
    ]
    ...
    with torch.no_grad():
        reference = [model([row])[0]["action_value"][0] for row in rows]  # [S_b, A] each
    ...
    for b, row_preds in enumerate(collected):
        ...
        assert torch.allclose(batched, reference[b], atol=1e-5), (
            f"row {b}: ragged batched decode diverged from unbatched decode"
        )
...
def test_empty_first_chunk_then_real_rows_match_unbatched(backbone_cls) -> None:
    ...
    chunk_lengths = [
        [3, 0],  # row 1 is entirely absent from the prefill
        [1, 2],
        [2, 3],
    ]
    ...
    with torch.no_grad():
        reference = [model([row])[0]["action_value"][0] for row in rows]
    ...
    for b in range(2):
        ...
        assert batched.shape == reference[b].shape
        assert torch.allclose(batched, reference[b], atol=1e-5), f"row {b} diverged"
...
def test_ragged_decode_fuzz(backbone_cls, seed) -> None:
    ...
    schedule[0, torch.argmax(schedule.sum(0))] += 1  # ensure a non-empty first call
    for call in range(num_calls):  # every call needs at least one non-empty row
        if schedule[call].sum() == 0:
            schedule[call, call % B] = 1
    totals = schedule.sum(0)
    for b in range(B):  # every row needs at least one step overall
        if totals[b] == 0:
            schedule[-1, b] = 1
    ...
    for b in range(B):
        ...
        assert batched.shape == reference[b].shape
        assert torch.allclose(batched, reference[b], atol=1e-5), (
            f"seed {seed} row {b} schedule {schedule.tolist()}: ragged decode diverged"
        )
...
def test_uniform_then_ragged_cached_decode() -> None:
    ...
    with torch.no_grad():
        ...
        cache = None
        # Uniform prefill (2 steps each), then ragged chunks.
