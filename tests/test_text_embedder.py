...
_DEFAULT_MODALITIES = [{'field': 'action', 'type': 'token'}, {'field': 'observation', 'type': 'text', 'format': 'observation={observation}'}, {'field': 'reward', 'type': 'text', 'format': 'reward={reward}', 'skip': 0.0}, {'field': 'done', 'type': 'text', 'format': 'done={done}', 'skip': 0}]

def _text_encoder(hidden_dim: int=8, **kwargs) -> TextEmbedder:
    vocab = 32
    emb = nn.Embedding(vocab, hidden_dim)
    nn.init.normal_(emb.weight, std=0.02)
    return TextEmbedder(hidden_dim=hidden_dim, tokenizer=_FakeTokenizer(), embed_tokens=emb, format=kwargs.pop('format', '<action={action},{observation},{reward},{done}>'), modalities=kwargs.pop('modalities', list(_DEFAULT_MODALITIES)), **kwargs)

def test_text_embedder_skip_omits_value_keeps_commas() -> None:
    enc = _text_encoder()
    batch = [[{'observation': 1, 'action': 0, 'reward': 0.0, 'done': 0}, {'observation': 2, 'action': 1, 'reward': 1.0, 'done': 1}]]
    ...
    with torch.no_grad():
        emb.weight.zero_()
        emb.weight[0] = 7.0
    enc2 = TextEmbedder(hidden_dim=8, tokenizer=_CaptureTok(), embed_tokens=emb, format='<action={action},{observation},{reward},{done}>', modalities=list(_DEFAULT_MODALITIES))
...
def test_token_modality_is_single_embed_row() -> None:
    ...
    with torch.no_grad():
        emb.weight.zero_()
        emb.weight[16] = 3.0
    enc = TextEmbedder(hidden_dim=D, tokenizer=_FakeTokenizer(), embed_tokens=emb, format='{action}', modalities=[{'field': 'action', 'type': 'token'}])
...
def test_text_embedder_rejects_learnable() -> None:
    emb = nn.Embedding(32, 8)
    try:
        TextEmbedder(hidden_dim=8, tokenizer=_FakeTokenizer(), embed_tokens=emb, format='{action}', modalities=[{'field': 'action', 'type': 'token'}, {'type': 'learnable'}])
...
def test_text_embedder_field_format_in_step_template() -> None:
    ...
    class _CaptureTok:
        ...
        def __call__(self, text: str, add_special_tokens: bool=False, return_tensors: str | None=None):
            ...
            return {'input_ids': torch.tensor([ids], dtype=torch.long)}
    emb = nn.Embedding(32, 8)
    enc = TextEmbedder(hidden_dim=8, tokenizer=_CaptureTok(), embed_tokens=emb, format='<{observation}|{action}>', modalities=[{'field': 'observation', 'type': 'text', 'format': 'o={observation}'}, {'field': 'action', 'type': 'text', 'format': 'a={action}'}])
...
def test_text_embedder_image_token_ids() -> None:
    ...
    def fake_image_tok(image):
        return [3, 4]
    emb = nn.Embedding(32, D)
    enc = TextEmbedder(hidden_dim=D, tokenizer=_FakeTokenizer(), embed_tokens=emb, image_processor=fake_image_tok, format='<{observation},{pixels}>', modalities=[{'field': 'observation', 'type': 'text', 'format': '{observation}'}, {'field': 'pixels', 'type': 'image'}])
...
def test_text_embedder_save_load(tmp_path) -> None:
    ...
    emb = nn.Embedding(32, D)
    nn.init.normal_(emb.weight, std=0.02)
    enc = TextEmbedder(hidden_dim=D, tokenizer=_FakeTokenizer(), embed_tokens=emb, pretrained=None, format='<action={action}>', modalities=[{'field': 'action', 'type': 'token'}])
    enc.pretrained = None
    model = Model(encoder=enc, backbone=IdentityBackbone(hidden_dim=D), heads=DiscreteActionValueHead(in_features=D, out_features=4, hidden_dim=D, num_layers=1))
