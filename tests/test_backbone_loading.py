...
def test_llama_backbone_loads_pretrained_checkpoint(tmp_path) -> None:
    ...
    with warnings.catch_warnings():
        ...
        backbone = LlamaBackbone(train_kernel="reference", decode_kernel="flex", dtype=torch.float32, use_norm=True, pretrained=tmp_path, num_layers=1)
...
def test_backbones_keep_and_load_the_final_norm(tmp_path) -> None:
    ...
    with warnings.catch_warnings():
        warnings.simplefilter('error')
        backbone = LlamaBackbone(train_kernel="reference", decode_kernel="flex", dtype=torch.float32, use_norm=True, pretrained=tmp_path)
    ...
    qwen = Qwen3Backbone(train_kernel="reference", decode_kernel="flex", dtype=torch.float32, use_norm=True, hidden_dim=8, num_layers=1, num_heads=2)
...
def test_llama_backbone_warns_on_unloaded_tensors(tmp_path) -> None:
    ...
    with pytest.warns(UserWarning, match='did not receive pretrained weights') as records:
        LlamaBackbone(train_kernel="reference", decode_kernel="flex", dtype=torch.float32, use_norm=True, pretrained=tmp_path, intermediate_size=32)
...
def test_llama_backbone_warns_on_unconsumed_pretrained_tensors(tmp_path) -> None:
    ...
    with pytest.warns(UserWarning, match='no matching backbone tensor') as records:
        LlamaBackbone(train_kernel="reference", decode_kernel="flex", dtype=torch.float32, use_norm=True, pretrained=tmp_path, attention_bias=False)
...
def test_num_layers_truncation_does_not_warn_about_dropped_layers(tmp_path) -> None:
    ...
    with warnings.catch_warnings():
        warnings.simplefilter('error')
        LlamaBackbone(train_kernel="reference", decode_kernel="flex", dtype=torch.float32, use_norm=True, pretrained=tmp_path, num_layers=1)
...
def test_qwen3_backbone_rejects_non_qwen3_checkpoint(tmp_path) -> None:
    ...
    with pytest.raises(ValueError, match="model_type='qwen3'"):
        Qwen3Backbone(train_kernel="reference", decode_kernel="flex", dtype=torch.float32, use_norm=True, pretrained=tmp_path)


def test_qwen3_backbone_direct_constructor_exposes_hidden_dim() -> None:
    backbone = Qwen3Backbone(train_kernel="reference", decode_kernel="flex", dtype=torch.float32, use_norm=True, hidden_dim=8, num_layers=1, num_heads=2)
    assert backbone.hidden_dim == 8
...
def test_qwen3_backbone_copies_pretrained_rope_parameters(tmp_path) -> None:
    ...
    backbone = Qwen3Backbone(train_kernel="reference", decode_kernel="flex", dtype=torch.float32, use_norm=True, pretrained=tmp_path)
