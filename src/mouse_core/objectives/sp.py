...
def sp_js(
    ...
    # → exp(log P) * (log P - log M). nan_to_num: -inf padding in info_metadata_q_star gives 0·(-inf) → NaN otherwise.
...
class SpObjective(Objective):
    """Supervised policy objective distilling ``info_metadata_q_star`` expert targets into action logits.
    ...
    Args:
        loss_type: Which distillation loss to apply.  ``"ce"`` uses the argmax of
            ``info_metadata_q_star`` as a hard label; the soft variants treat it as a
            distribution.
        temperature: Softmax temperature applied to ``info_metadata_q_star`` before soft losses
            (ignored for ``"ce"``).
    ...
    def __call__(
        ...
        q_targets = objective_data["info_metadata_q_star"].reshape(-1, A).to(dtype=logits.dtype)

        if q_targets.shape[0] == 0:
            raise ValueError("SpObjective: batch is empty (no tokens).")
        if not torch.isfinite(q_targets).all():
            raise ValueError("SpObjective: info_metadata_q_star contains non-finite values (NaN or inf).")
