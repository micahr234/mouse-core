...
class SvObjective(Objective):
    ...
    Only finite entries of ``info_metadata_q_star`` participate in the loss; ``-inf``
    ...
    def __call__(
        ...
        q_targets = objective_data["info_metadata_q_star"].reshape(-1, A).to(dtype=logits.dtype)  # [B*S, A]
        ...
        if not finite_mask.any():
            raise ValueError("SvObjective: info_metadata_q_star contains no finite values (all NaN or -inf).")
